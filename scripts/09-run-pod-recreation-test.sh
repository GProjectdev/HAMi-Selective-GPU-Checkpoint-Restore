...
export RESTORE_CHECKPOINT_URI RESTORE_DATA_URI RESTORE_SOURCE_POD_UID TARGET_NODE
...
append_summary "# Pod A Restore" "" "- restore pod: hami-pod-a-restored" "- checkpoint-uri: ${RESTORE_CHECKPOINT_URI}" "- data-uri: ${RESTORE_DATA_URI:-default .tar -> .blob}" "- source-pod-uid: ${RESTORE_SOURCE_POD_UID}" "- target-node: ${TARGET_NODE}"
